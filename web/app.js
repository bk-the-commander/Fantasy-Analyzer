...
const SITE = {
  ...
  /* Watermark stamped on cards, boards and charts. */
  watermark: 'Generated by BK',
  owner: 'BK',

  /* ---- CONTACT — fill these in and the Contact page wires itself up. ----
   * Left blank on purpose: publishing an address is the owner's call, not a
   * default. An empty string simply hides that row. */
  contactEmail: '',          // e.g. 'you@yourdomain.com'
  twitter: '',               // handle without the @
  instagram: '',
  discord: '',               // invite URL

  /* Shown on About / Privacy / Terms. */
  legalEntity: '',           // e.g. 'Fantasy Analyzer' or a registered LLC
  jurisdiction: '',          // e.g. 'New York, USA' — governing law for Terms
...
function renderFooter() {
  ...
  $('#siteFoot').replaceChildren(el('div', { class: 'foot-inner' },
    el('div', { class: 'foot-brand', html: `FANTASY <em>ANALYZER</em>` }),
    ...
    el('div', { class: 'foot-mark' }, SITE.watermark)));
}
...
function viewContact() {
  ...
  if (SITE.twitter) {
    rows.push(el('a', { class: 'btn', href: `https://twitter.com/${SITE.twitter}`,
                        target: '_blank', rel: 'noopener' }, `@${SITE.twitter}`));
  }
  if (SITE.instagram) {
    ...
    el('div', { class: 'panel' },
      ...
      prose(
        ...
        rows.length
          ...
          : p(`<span class="meta">Contact details are being finalised — check
               back shortly.</span>`),
...
function warnUnsetConfig() {
  ...
  if (missing.length) {
    console.info(
      `[Fantasy Analyzer] Unset SITE fields: ${missing.join(', ')}. ` +
